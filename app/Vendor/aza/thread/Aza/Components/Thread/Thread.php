...
abstract class Thread
{
	...
	private static function evMasterLoop($base)
	{
		...
		if(!$base->getIsInLoop()){
			...
			// If $base is in loop, now system will throw an exception according to the proginal process
			// In order to let system run without stop, we add this {if...else...} statement here,
			// and what we do is to stop the current loop, free loop resources and manually start a new one.
			// We start new main event loop by throwing an exception here and the thread component will catch it,
			// and this action will stop the thread component and force the thread component to restart everything all over again.
			$base->cleanAllLoops();
			$base = null;
			throw new Exception(
				"Main loop is corrupted"
			);
		}
	}
			...
			if ($this->isAlive()) {
				...
				if ($wait) {
					...
					if (!$st) {
						...
						$this->isStopping = false; // make sure the following recursive function will work
	...
	protected function shutdown()
	{
		if ($this->isChild) {
			...
			// Send signal to kill child process instead of using "exit",
			// because "exit" can break the DB connection and cause "Uncaught exception 'PDOException' with message 'SQLSTATE[HY000]:
